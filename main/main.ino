...
void loop() {
  ...
  if (start) initial(); //in initial, everything is hard coded until enter white line loop
  else if (drop) drop(); //in drop, will do turning, dropping, backing, and change holding bool to false
  else {
    //if within 5 cm of block lower speed of motor
    if (frontUS.reading(analogRead(FRONT_PIN)) < 5) lowerSpeed();
    else normalSpeed();
    ...
    if (frontUS.reading(analogRead(FRONT_PIN)) < 1 && !pickup) pickupAll();

    //if in tunnel drive in tunnel else follow line
    if (topIR.reading(analogRead(TOP_PIN)) < ceilingThreshold) tunnelDriving();
    else followLine();
    ...
    if (holding && (magnet && noOfWhiteLines >= 3 || !magnet && noOfWhiteLines == 1)) drop = true;
